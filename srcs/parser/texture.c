...
static int	load_xpm(char *path, t_image *img, t_params *params)
{
	int		local_endian;
...
static int	load_texture(t_params *tmp, t_image *img, char *path, int parser)
{
	int fd;
	...
	while (*path && (*path == ' ' || *path == '\t'))
		path++;
	if (*path == 0)
		return (parse_error("Path texture missing or unknown."));
